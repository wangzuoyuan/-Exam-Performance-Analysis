...
test('student views preserve scoped identity history and A4 report isolation', () => {
  ...
  assert.match(profile, /高一 · 九科/)
  assert.match(profile, /高二\/高三 · 6选3/)
  assert.match(profile, /nine_trend\?: MainTrendPoint\[\]/)
  assert.match(profile, /grade_score\?: number \| null/)
  assert.match(profile, /activeStage === 'grade1'/)
  assert.match(profile, /isElectiveInStage\(activeStage, spec\.subject\)/)
  assert.match(profile, /等级分缺失，原始分仅作参考，不进入趋势线/)
  assert.match(profile, /const value = total\?\.score \?\? null/)
