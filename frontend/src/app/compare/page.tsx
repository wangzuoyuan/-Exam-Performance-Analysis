...
type SortKey = 'class_num' | 'main_total_avg' | 'diff' | 'rank'
...
export default function ComparePage() {
  ...
  const metricValueByClass = useMemo(() => {
    ...
    if (metricDef.source === 'subject_pair') {
      ...
      for (const row of examDetail?.class_averages || []) {
        ...
        if (typeof value === 'number') values.set(row.class_num, value)
      }
      return values
    }
    ...
    return values
  }, [currentCompare, examDetail, metricDef])
  ...
  const chartData = useMemo(() => {
    const classes =
      currentCompare?.classes ||
      (examDetail?.class_averages || []).map(row => ({ class_num: row.class_num }))
    if (classes.length === 0) return []
    const sorted = [...classes].sort((a, b) => a.class_num - b.class_num)
    return sorted.map(c => {
      const row: Record<string, number | string> = {
        classLabel: `${c.class_num}班`,
        class_num: c.class_num,
      }
      if (metricDef.source === 'subject_pair') {
        const avgRow = examDetail?.class_averages.find(item => item.class_num === c.class_num)
      ...
      const value = metricValueByClass.get(c.class_num)
      if (typeof value === 'number') row[chartBars[0].key] = Number(value.toFixed(1))
      return row
    })
  }, [chartBars, currentCompare, examDetail, metricDef, metricValueByClass])

  // 排名表数据
  const rankRows = useMemo(() => {
    const classes = Array.from(metricValueByClass.entries()).map(([class_num, value]) => ({
      class_num,
      class_type: currentCompare?.classes.find(c => c.class_num === class_num)?.class_type,
      value,
    }))
    if (classes.length === 0) return []
    const total = classes.reduce((s, c) => s + c.value, 0)
    const gradeAvg = total / classes.length

    const withMeta = classes.map(c => ({
      class_num: c.class_num,
      class_type: c.class_type,
      main_total_avg: c.value,
      diff: c.value - gradeAvg,
    }))
    ...
    return sorted
  }, [currentCompare, metricValueByClass, sortKey, sortDir])
        ...
        >
          <div className="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              <span className="text-sm text-slate-500">本班</span>
              {targetClass ? (
                <Badge
                  variant="secondary"
                  className="bg-brand-50 text-brand-700 hover:bg-brand-50"
                >
                  {formatClassLabel(currentGrade, targetClass)}
                </Badge>
              ) : (
            ...
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Tooltip>
                ...
                <TooltipContent side="left" className="max-w-[220px]">
                  当前按{metricDef.desc}对比；本班柱子高亮，其它班柱子半透明。
                </TooltipContent>
              </Tooltip>
            </CardHeader>
            <CardContent>
                ...
                <div className="overflow-hidden rounded-lg border border-slate-200">
                  <Table>
                    ...
                    <TableBody>
                      {rankRows.map(row => {
                        const isTarget =
                          targetClass !== null && row.class_num === targetClass
