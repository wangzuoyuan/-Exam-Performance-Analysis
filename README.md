...
当前版本采用面向班主任工作流的渐进式前端设计：全站常驻“年级 + 已绑定行政班”作用域，仪表盘、考试、学生、作业、AI 与备份共享同一范围；切换班级后不会回退到固定班号。桌面、平板和手机使用同一套 Direction B 设计系统，并统一提供加载、空数据、错误、首次使用和部分成功状态。

## v2.2.4 跨届学号撞号防护

- 高二分班重新编号后新学号可能撞历史旧学号：启动时自动检测，把学号名下别人的历史成绩改写为内部命名空间学号（显示不变），并按姓名自动建立跨学年身份链。
- 成绩上传与花名册录学号增加防呆，学号与历史数据姓名不符时明确拒绝，杜绝画像串人。
- 花名册成员增删收口到「学生管理」页，作业设置页只保留查看与排除统计。

## v2.2.3 作业多学期管理

- 作业设置页新增「历史学期」：保存往期学期、一键设为当前学期，看板、预警、相关性、学生作业概况与本周关注随学期整体切换。
- 未配置学期时按日期自动推算当前学期（9~1 月第一学期、2~7 月第二学期、8 月归新学年），跨年自动滚动，换学期不再出现看板清零。
- 旧的单学期配置升级时自动转为历史学期，不抢占当前学期。

## v2.2.2 学生画像学段口径

- 学生画像页新增「高一 · 九科」与「高二/高三 · 6选3」切换，默认显示最新学段。
- 高一展示主三门、五门、九门与九科百分位；高二/高三展示主三门、+3、3+3、语数英百分位与选考等级分。
- 历次成绩明细按学段使用双列口径，选考科区分原始分与等级分；未选或缺考显示「—」。
- 保留跨学年主三门趋势作为唯一跨学段比较口径，并修复中文临时学号画像路由解码。

## v2.2.1 学生管理

- 新增独立的「学生管理」页面：可集中维护姓名、学号、性别、座号、备注和在班状态，并同时提供桌面表格与手机卡片视图。
- 支持安全纠正录错学号、补录新学年学号、归档转班或毕业学生；跨学年旧学号与关联历史保留不断档。
- 删除、合并前均展示影响范围；涉及成绩、作业或谈话档案时必须二次确认并自动备份，避免误删。
- 提供重复学生合并、身份回填和按当前班级隔离的变更日志；所有写操作由服务端强制限定在教师已绑定的年级和行政班。

## v2.2.0 行内批量确认同名学生

- 「逐人判定」改为行内批量确认，不再为每位同名学生重复弹窗；安全匹配默认选中，可一键确认全部。
- 多候选、新学生和稍后处理都可直接在当前表格完成；服务端再次整批核验，发生冲突时整批拒绝，绝不部分写入。
- 每次批量确认都会生成可撤销快照，仅撤回本批新建的关联，不影响此前已有关系或后续补录。
- 切换年级或行政班会清理旧范围的预览与选择，避免跨范围误提交。

## v2.1.0 先建名册、后补学号

- 升级换届支持只录姓名：系统生成当前年级与班级专属的临时学号，可立即使用作业花名册、缺交记录和学生检索。
- 拿到正式学号后，在原输入框粘贴“学号,姓名”即可安全替换临时学号，作业、特殊记录、谈话档案和跨学年身份接续关系同步迁移。
- 从成绩派生名册也复用同一事务，不会生成临时、正式两条重复学生；学号、姓名、班级或身份冲突时整批拒绝并保留原数据。
- 可按明确条件收编旧版错误写入的姓名行，同时保持高一名册、成绩和未确认的同名身份不变。
- 新增只读 MCP 服务，供 Hermes 等客户端在独立令牌和 Host/Origin 防护下查询班主任版数据。

## v2.0.0 重设计

- 全站视觉与响应式布局按班主任日常任务重构，保留所有原路由和真实业务口径。
- 新增持久化行政班作用域，彻底移除运行时固定 6 班回退。
- 仪表盘、考试、学生、班级对比、作业、AI、上传、换届和备份统一状态与交互。
- 学生详情与 A4 一页纸增加当前班权限校验，打印内容严格限定当前学生。
- 建立 `npm run test:ui` 契约测试，并将后端测试完全隔离于真实用户数据库。
...
- **学生画像页**：按高一与高二/高三学段口径展示主三门、五门、九门、+3 / 3+3、单科历史和历次考试明细。
...
- **升级换届（跨学年身份接续）**：学生升入高二换新学号后，系统仍能识别为同一个人，跨学年趋势、缺交历史、谈话档案自动接续，不因换号断档。
...
- **AI 对话助手**：支持 Anthropic Messages API 和 OpenAI Chat Completions 兼容接口，使用当前已绑定班级和只读工具查询本地成绩、作业与档案数据后回答（如「本班缺交最多的 5 人成绩排名如何」「哪门课缺交最拖成绩」「结合最近谈话帮我准备和某某的谈话提纲」）。
...
- 可选范围只来自教师在高一、高二、高三已绑定的行政班。
- 前端请求显式携带当前 `grade + class_num`；省略班号的后端调用从教师当前绑定班级解析。
- 未绑定班级时页面显示配置提示，不猜测、不静默回退到任何固定班级。
- 快速切换范围时会取消旧请求，避免上一班的数据覆盖当前班。
- AI 工具、作业统计、学生名单和一页纸报告遵循同一范围校验。

## 升级换届（跨学年身份接续）

学生从高一升入高二会换新学号。本功能用一个「身份层」把同一人的多个学号链接起来，让跨学年的成绩趋势、缺交记录、谈话档案自动接续。

升级后通过**换届向导**一次性完成接续：

1. **绑定高二班**：把班主任目标班切到高二新班号。
2. **建高二名册**：在同一个输入框粘贴「学号,姓名」或仅「姓名」——还没有新学号时先按姓名建册（系统生成 TMP- 临时学号），立即可用作业花名册 / 缺交录入；之后拿到正式学号再粘贴一次「学号,姓名」，作业、特殊记录、谈话档案会自动跟随到正式学号。高一名册与新名册并存（互不影响）。
3. **行内批量判定四态**：在同一张表内核对高二与高一同名学生；安全项可一键确认，其余可选同一人、新学生或稍后处理。
4. **导入对照表**：若已有高一↔高二学号对照表，可一次性批量建链接。
5. **导入高一成绩**：把高一的历史成绩导入，仅出现在该生个人画像（不参与全年级排名、班均、段位计算）。
6. **切作业看板年级**：作业看板默认只看当前学年名册，可随时切换查看年级。

学生页和「按人去重」的学生检索都会合并同一人的全部学号；班级看板、段位、缺交统计仍按当年行政班单年口径。
...
| GET | `/api/students/{id}` | 学生跨学年画像：含主三门/五门/九门/+3/3+3趋势；单科趋势带原始分、等级分与年级百分位 |
...
后端测试会在临时目录创建独立数据库和备份目录，不读取或修改真实 `~/.exam-tracker` 数据。
